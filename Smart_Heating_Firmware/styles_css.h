...
#tab {
  ...
  padding: clamp(0.1rem, 0.1rem + 1vw, 0.5rem);
  max-width: calc(100vw - clamp(2rem, 5rem + 10vw, 20rem));
}
...
.container button#delete.disabled {
  color: transparent;
  background-color: transparent;
  border: none;
  pointer-events: none;
...
.container .timer-buttons button.disabled,
.container .timer-buttons button#delete.disabled {
  display: none;
}

/* Media query for smaller screens (300x500 px) */
@media (max-width: 500px) {
  ...
  .toggle__circle {
    width: 1.2rem;
    height: 1.2rem;
    margin-top: calc(3.5rem - (0.2rem * 2) - 1.2rem);
  }
}

/* Media query for horizontal orientation (small screens) */
